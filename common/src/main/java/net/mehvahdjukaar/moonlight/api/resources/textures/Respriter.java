    ...
    private interface FrameColorRemapper {
        ...
        static FrameColorRemapper of(Palette originalPalette, int originalFrameCount,
            ...
            if (originalFrameCount != 1 || invalidSize) {
                if (invalidSize) {
                    Moonlight.crashIfInDev("Respriter tried to generate a texture with more frames than the given palettes which were also >1. The 2 must match. Sizes: " +
                            targetFrameCount + " vs " + targetPalettes.size());
                }
