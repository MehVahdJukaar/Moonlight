...
public class DatePickerPopup extends AnchoredPopup {
    ...
    protected void renderContent(GuiGraphics graphics, int x, int y, int mouseX, int mouseY) {
        ...
        graphics.drawCenteredString(font, "<", x + PAD + CELL_W / 2, textY, overPrev ? TEXT : TEXT_SECONDARY);
        graphics.drawCenteredString(font, ">", x + width - PAD - CELL_W / 2, textY, overNext ? TEXT : TEXT_SECONDARY);
        graphics.drawCenteredString(font, month.getDisplayName(TextStyle.FULL, GuiHelper.currentLocale()), x + width / 2, textY, TEXT);
